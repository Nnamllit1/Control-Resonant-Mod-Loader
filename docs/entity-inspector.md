...
The experimental inspector records the player entity's component identities and known movement state without requesting gameplay changes. It is the first step toward mapping live model, material, and physics relationships. It currently identifies selected rendering components by hash; it does **not** decode their payloads or resolve model/material resource names.
...
- Up to 512 component hashes, with selected known names; `truncated` reports the limit.
